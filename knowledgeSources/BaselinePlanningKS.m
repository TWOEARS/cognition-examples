...
classdef BaselinePlanningKS < AbstractKS
    ...
    methods
        % the constructor
        function obj = BaselinePlanningKS(robot)
            obj = obj@AbstractKS();
            % call each frame
            ...
            obj.taskStack.push('rotateScanExploration');
        ...
        function execute(obj)
            ...
            % get global position reliability
            globalPositionReliability=inf;
            try
                globalPositionReliability=...
                    obj.blackboard.getLastData(...
                                'globalPositionReliability').data;
            ...
            if ~ obj.taskIsPending
                ...
                if (~obj.taskStack.empty())
                    ...
                    if strcmp(command,'rotateScanExploration')
                        ...
                        % note: tasks have to be inserted in inverse order!!!
%                           obj.taskStack.push('rotate:-45');
%                           obj.taskStack.push('rotate:90');
%                           obj.taskStack.push('rotate:-45');
%                           
%                         
%                         obj.taskStack.push('rotate:+90');
%                         obj.taskStack.push('rotate:-45');
                    ...
                    if strcmp(command,'triggerAlert')
                        ...
                        for i=1:size(sources,1)
                            if strcmp(sources{i,1}.name,'Source005')
                                ...
                                output=sprintf('setObjectDisplayStatus,%s,%s\r\n',sources{i,1}.name,'emitting');
                    ...
                    if strcmp(command,'setupRescuePlan')
                        ...
                        for i=1:size(auditoryObjectHyps,1)
                            hypScore=auditoryObjectHyps{i,1}.hazardScore;
                    ...
                    if strcmp(command,'rescuePrioritizedEntity')
                        ...
                        for i=1:size(auditoryObjectHyps,1)
                            ...
                            if (hypothesis.rescueOrderNumber==1)
                                ...
                                if hypothesis.hazardScore>0
                                
                                    str=sprintf('rescue:%s:%f:%f',hypothesis.label,...
                                        hypothesis.currentPositionEstimate(1,1),...
                                        hypothesis.currentPositionEstimate(1,2));
                                    obj.taskStack.push(str);
                                else
                                    % no more victims to be rescued, go to
                                    % idle position
                                    obj.taskStack.push('setIdleMode');
                    ...
                    if strcmp(command,'rescue')
                        
                        % evacuate an entity from a scenario
                        currentSuperTask=command;
                        ...
                        % irrelevant
                        position=[str2double(param2),str2double(param3),0];
                        obj.wayPoints(param1)=position;
                        ...
                        obj.robot.deleteObstacleForSource(param1);
                        
                        % navigate towards entity
                        str=sprintf('navigate:%s',param1);
                        obj.taskStack.push(str);
                    ...
                    if strcmp(command,'rotate')
                        ...
                        obj.robot.robotInterface.rotatePlatformRelative(str2double(param1),15.0);
                        obj.taskIsPending=true;
                    end
            ...
            if strcmp(currentSuperTask,...
                            'standardExploration') && ...
                    globalPositionReliability<1e-3
            ...
            if strcmp(currentSuperTask,...
                            'setupRescuePlan') && obj.taskStack.empty()
                   
                % rescue the next source in the rescue list
